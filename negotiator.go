...
func (n *Negotiator) StartConnection(opts ConnectionOptions) error {
	...
	if opts.Originator {
		...
		// OFFER
		err = n.handleSDP(ServerMessageTypeOffer, opts.SDP)
		if err != nil {
			return err
		}
	}
...
func (n *Negotiator) setupListeners(peerConnection *webrtc.PeerConnection) {
	...
	peerConnection.OnICEConnectionStateChange(func(state webrtc.ICEConnectionState) {
		...
		case webrtc.ICEConnectionStateClosed:
			...
			n.connection.Emit(ConnectionEventTypeError, fmt.Errorf("Connection to %s closed", peerID))
			n.connection.Close()
			break
		...
		n.connection.Emit(ConnectionEventTypeIceStateChanged, peerConnection.ICEConnectionState())
	})
...
func (n *Negotiator) handleSDP(sdpType string, sdp webrtc.SessionDescription) error {
	...
	// sdpType == OFFER
	if sdpType == ServerMessageTypeOffer {
		err := n.makeAnswer()
		if err != nil {
			return err
		}
